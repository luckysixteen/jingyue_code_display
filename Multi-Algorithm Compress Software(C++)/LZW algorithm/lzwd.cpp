...
static int LZWInlnit ( void )
{
	havenibble = 0;
	if ( ! DecodeBufferSize )
	{
		DecodeBufferSize = 2000;
		DecodeBuffer = (CodeType*)malloc( DecodeBufferSize );
...
static unsigned LZWLoadBuffer ( unsigned count, CodeType code )
{
	...
	while ( code >= PRESET_CODE_MAX )
	{
		...
		{
			DecodeBuffer =(CodeType*)realloc ( DecodeBuffer, DecodeBufferSize + 2000 );
			if ( ! DecodeBuffer )
			{
				/* out of memory */
				DecodeBufferSize = 0;
...
priming:
	...
	while ( ! ( retval = LZWIn ( infile )))
	{
		...
		if ( freecode < MAX_CODE )
		{
			...
			{
				unsigned cnt;
				printf( " just added code %5u: \"",freecode - 1 );
...
int main ( int argc, char *argv[] )
{
	...
	if ( argc != 3 )
	{
		fprintf ( stderr, "Usage: lzwd infile outfile\n" );
		return EXIT_FAILURE;
	}
