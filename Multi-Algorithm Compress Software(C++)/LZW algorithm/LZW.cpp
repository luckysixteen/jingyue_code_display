...
#if defined( HASHHITS )
    SHOWHASH( memset ( HashHits, 0,DICTIONARY_SIZE * sizeof ( unsigned long )); )
#endif
    SHOWHASH( FreeMax = 0; )
}
...
static void LZWOutInit( FILE *outfile )
{
    ...
    SHOWHASH( for ( i = 0; i < DICTIONARY_SIZE;	i++)
             HashHital[i] = 0; )
#endif
    dictionary = malloc ( DICTIONARY_SIZE *sizeof( struct Dictionary ));
}
static int LZWOutFlush ( FILE *outfile )
{
	return 0;
...
static int LZWOut ( FILE *outfile, CodeType code )
{
	if ( fwrite ( fccode, sizeof( CodeType ), 1, outfile ) != 1 )
		return 4;
	LZWTotBytesOut+=2;
	LZWCurrBytesOut+=2;
	return 0;
...
static IndexType LZWFind ( CodeType currcode, int in )
{
    ...
    for ( ;; )
    {
        if ( dictionary[ ndx ].code == UNUSED_CODE )
            break;
        if ( dictionary[ ndx ].parent == currcode && dictionary[ ndx ].c == in )
            break;
        ndx +=step;
	...
	else
	{
		...
		while ((in = fgetc ( infile )) != EOF )
		{
			...
			if ( dictionary[ idx ].code==UNUSED_CODE )
			{
				...
				{
					unsigned ratio= ( LZWCurrBytesOut * 100 ) / LZWCurrBytesIn;
#if defined( HASHSTATS )
					printf( "Inputs %lu, Outputs %lu, Overall Ratio; %lu%%, LZWTotBytesIn,LZWTotBytesOut,( LZWTotBytesOut * 100 ) / LZWTotBytesIn );
                           printf( "This frames %u%%\n", ratio );
#endif
                           LZWCurrBytesIn = 0;
                           LZWCurrBytesOut = 0;
                           ...
                           else /* we match so far¡ªkeep going */
                           ...
                           if ( ! retval )
                           {
                               ...
                               printf ( "The output file contained %lu data bytea\n",LZWTotBytesOut );
                               if ( LZWTotBytesIn != 0 )
                                   printf ( "Output file is %lu%% the size of input file\n",( LZWTotBytesOut * 100 ) / LZWTotBytesIn );
                           }
#endif
#if defined( HASHSTATS )
                           {
                           ...
                           int main( int argc, char *argv[] )
                    {
                        ...
                        if ( argc != 3 )
                        {
                            printf( "Usage: lzwc infile outfile\n" );
                            return EXIT_FAILURE;
                        }
