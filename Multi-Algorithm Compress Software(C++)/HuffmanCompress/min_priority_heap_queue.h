...
public:
	...
	MinPriorityHeapQueue(const MinPriorityHeapQueue<ElemType> &copy);		// 复制构造函数模板
...
// 操作结果：如果优先队列非空，那么删除堆顶元素，并用e返回其值，返回SUCCESS,
//	否则返回UNDER_FLOW，
{
	if (!Empty())
	{	// 优先队列非空
...
// 操作结果：如果队列非空，那么用e返回堆顶元素，返回SUCCESS,
//	否则返回UNDER_FLOW，
{
	if (!Empty())
	{	// 优先队列非空
...
// 操作结果：由优先队列copy构造新优先队列——复制构造函数模板
{
...
// 操作结果：将优先队列copy赋值给当前优先队列——重载赋值运算符
{
	if (&copy != this)
	{
		delete []elem;								// 释放空间
