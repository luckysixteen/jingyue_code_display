...
typedef enum {
//节点类型_19种
//包括：int、id、void、数值、变量声明、数组声明、函数声明、函数声明参数列表、函数声明参数、复合语句体、if、while、return、赋值、运算、数组元素、函数调用、函数调用参数列表、未知节点
    IntK, IdK, VoidK, ConstK, Var_DeclK, Arry_DeclK, FunK, ParamsK, ParamK, CompK, Selection_StmtK, Iteration_StmtK, Return_StmtK, AssignK, OpK, Arry_ElemK, CallK, ArgsK, UnkownK} Nodekind;
...
//语法树treeNode定义
//包括：子节点、兄弟节点、所处行号、节点类型、属性、表达式返回类型
	struct treeNode *  child[MAXCHILDREN];
	struct treeNode *  sibling;
	int lineno;
    Nodekind nodekind;
	union {
        TokenType op; int val; string name;
    }attr;
    ExpType type;
} TreeNode;

ofstream fout_Tree("/Users/Jen/Documents/Development/compile_fjy/compile_fjy/tokenTree.txt");
